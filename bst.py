...
class BinarySearchTree(Generic[K, I]):
    ...
    def get_successor(self, current: TreeNode) -> TreeNode:
        """
            ...
            complexity: O(D) where D is the depth of the next successor from the current node
        """
    ...
    def get_minimal(self, current: TreeNode) -> TreeNode:
        """
            ...
            :complexity: 
                best-case: O(1) when the smallest key is the next node down
                worst-case: O(D) where D is the depth of the tree and the smallest key
